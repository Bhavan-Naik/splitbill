...
for k in range(places_count):
    ...
    for z in range(size):
        new_row = [items[z]] + [0] * n
        print(comment)
        vals = input("Enter assigned number of people to split for " + items[z] + " seperated by comma: ")
        vals = vals.split(",")
        m = len(vals)
        equal_flag = 'y'
        if m != 1:
            equal_flag = input("Do you want to split the bill among " + str(m) + " members equally?[y/n]: ")
        if equal_flag[0] == 'N' or equal_flag[0] == 'n':
            unequal_splits = []  # What if it does not equal to 100
            for i in vals:
                split_perc = float(input("Enter the split percentage for " + group[int(i) - 1] + ": "))
...
filename = "{}_bill.csv".format(time_script_run)
